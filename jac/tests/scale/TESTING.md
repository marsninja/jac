...
Build the compiler and packaged runtime from the current branch:
...
For subsequent compiler or scale edits, rebuild the development image and
...
### 1.2 Verify the selected compiler image

```bash
jac -c 'import jaclang; from jaclang.compiler.driver.jir import running_compiler_digest; print(jaclang.__file__); print(running_compiler_digest())'
```

The path identifies the loaded image; the digest identifies its compiled
contents. Rebuild after source edits. Invalid image paths fail at startup.
Unset `JAC_COMPILER_IMAGE` to validate the packaged binary.
...
```bash
jac test --jobs 0 jac/tests/scale/<group>/test_<name>.jac
```
...
CI runs six groups under `jac/tests/scale/`, and they map to
