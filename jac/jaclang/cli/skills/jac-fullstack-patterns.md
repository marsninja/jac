...
- **Endpoints register from their declarations, not from the entry import.** Every `def:pub` / `def:protect` (and `walker:pub` / `walker:protect`) in a module the app compiles is served at `/function/<name>` (walkers at `/walker/<name>`), whether or not `main.jac` imports it. The entry import is still how server code *uses* an endpoint in-process; it is no longer how the endpoint comes to exist. **404/405 on RPC = the declaration is plain (private) or belongs to another app.**
...
- **A separate provider is an app, not an import form.** Giving a module its own `[apps.<name>] kind = "service"` table makes every other app's imports of its walkers / `def:pub` functions typed-async bridge stubs (`await` them - `E1042` otherwise) and lets it run as its own process (`--fleet`, deploy) or colocated (`jac run`, the default); session cookies don't cross, so an anonymous chain reaching a `def:protect` gets `401 Unauthorized`. The cross-app surface is every exposed declaration, `:pub` or `:protect` (see `jac-sv-microservices`).
...
- **The served app's client is at `/`.** `jac run <app>` serves that app's `app` page (or `index`/`main`/`home`/`root`) at `/`; a project with no client page keeps `/` as the JSON API index. Sibling client apps in the workspace with a built bundle (`jac build --all` -> `dist/<app>/`) are mounted at `/cl/<app>/`. The server's SPA catch-all serves the app HTML for clean URLs (BrowserRouter), excluding API prefixes (`cl/`, `walker/`, `function/`, `user/`, `static/`).
...
- **Start with `jac run --dev main.jac`** (`jac serve` is not a command - it hard-errors). HMR reloads only client modules - server-module / `glob` changes need a full restart (endpoints and `glob`s evaluate once at server boot). Kill stale `jac run` processes first: a held port makes the new server grab the next port while Vite's proxy still points at the old one → all RPC calls fail. `pkill -f "jac run"` then restart. `jac run` exits when stdin closes - launch background/long-running servers with `< /dev/null`.
...
- **Build failures print `E7xxx` diagnostics at a Jac location** (E7001 no such export, E7002 unresolved import, E7004 Vite rejected a module, E7005 the build produced no loadable module), rendered like `jac check` output; set `JAC_DEBUG=1` (or `[client] debug = true`) for raw Vite output. Compiled JS for inspection: `.jac/client/compiled/`.
...
- `jac-sv-endpoints` - writing `def:pub` / `def:protect` endpoints and walker endpoints
