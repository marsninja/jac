...
- **Exposed functions and walkers (`:pub` or `:protect`) in a server-anchored module stay server endpoints.** A client reference never inlines them into the bundle; the call compiles to the auto-RPC bridge (`await save_note(...)`, `result = root spawn add_task(title=t);` - see `jac-fullstack-patterns`). The access tag on the declaration is the cross-space contract: `:pub` bridges anonymously, `:protect` bridges with the caller's token, plain and `:priv` never bridge (E5082 from client code, E5106 from another app). (A `def:pub` whose own body carries JSX is client by the structural rule - that is how markerless `def:pub app` and components work. And in a module with NO server anchor at all, a pure `def:pub` can be pulled client wholesale - there is no server side to bridge to.)
...
Pins feed the solver exactly like the old markers did: a pinned element is immovable, and everything else re-solves around it. Pins decide only *where* code runs; *who may call it* is decided by the declaration's access tag alone, so a module-level `"server"` pin does not change which imports bridge. Pins are part of the program: changing them invalidates the compilation cache and shows up in `--placements` evidence as `pinned 'server' ([placement.pins])`.
...
- **`def:pub` + JSX body = client component; `def:pub` / `def:protect` in a server-anchored module = server endpoint**, RPC-bridged when the client calls it. A plain `def` there is a private helper.
